...
$selectedOrg = $_GET['org'] ?? 'MSS'; // Default to MSS
...
WHERE UPPER(TRIM(i.code)) = 'MSS' OR UPPER(TRIM(i.institution_name)) LIKE '%MSS%' OR UPPER(TRIM(i.institution_name)) LIKE '%MERCHANT SHIPPING%'";

$result = mysqli_query($conn, $sql);
$projects = [];
if ($result) {
    while ($row = mysqli_fetch_assoc($result)) {
        $row['institution_code'] = 'MSS';
...
<style>
    ...
    .dashboard-header {
        background: linear-gradient(135deg, #0f766e 0%, #115e59 100%); /* MSS specific teal gradient */
    ...
    .floating-filter-btn {
        ...
        box-shadow: 0 10px 25px rgba(20, 184, 166, 0.4); display: flex; align-items: center; gap: 10px;
        transition: 0.2s;
    }
    ...
    .field-group label { font-size: 11px; font-weight: 900; color: #134e4a; text-transform: uppercase; display: flex; align-items: center; gap: 8px; }
    ...
    .project-card:hover { transform: translateY(-5px); border-color: #0d9488; box-shadow: 0 15px 30px rgba(0,0,0,0.1); }

    .inst-badge { position: absolute; top: 18px; right: 18px; padding: 5px 12px; border-radius: 8px; font-size: 10px; font-weight: 900; color: white; display: flex; align-items: center; gap: 5px; background: #0f766e; }
    ...
    .card-title { font-size: 17px; font-weight: 900; color: #134e4a; margin: 5px 0; line-height: 1.3; padding-right: 75px; min-height: 44px; }
    
    .card-meta-row { display: flex; flex-wrap: wrap; gap: 6px; margin-bottom: 5px; }
    .meta-tag { background: #f0fdfa; padding: 4px 10px; border-radius: 6px; font-size: 9px; font-weight: 800; color: #134e4a; border: 1px solid #99f6e4; display: flex; align-items: center; gap: 5px; }
    ...
    .matrix-dark { background: #134e4a !important; color: white !important; }
    .matrix-dark .sub-label { color: #99f6e4 !important; border-top-color: rgba(255,255,255,0.1) !important;}

    .bar-group { width: 100%; }
    .bar-label { display: flex; justify-content: space-between; font-size: 10px; font-weight: 900; margin-bottom: 4px; align-items: center; color: #134e4a; }
...
<div class="dashboard-header">
    <h1><i class="fa-solid fa-anchor"></i> MERCHANT SHIPPING PROJECTS</h1>
    <div style="font-size: 11px; font-weight: 900; color: #fbbf24;">
        <i class="fa-solid fa-calendar-check"></i> Q1 2026 | MSS <?= ($selectedDiv != 'all') ? " : ".urldecode($selectedDiv) : "" ?>
    </div>
</div>
...
<nav class="filter-drawer" id="drawer">
    <div class="drawer-content">
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:30px;">
            <h2 style="margin:0; font-size:20px; color: #134e4a;"><i class="fa-solid fa-sliders"></i> Filter Menu</h2>
        ...
        <div class="field-group">
            <label><i class="fa-solid fa-magnifying-glass" style="color:#0d9488;"></i> Search Project</label>
        ...
        <button onclick="toggleDrawer()" style="width:100%; background:#134e4a; color:white; border:none; padding:18px; border-radius:12px; font-weight:900; cursor:pointer; margin-top:10px; font-size:15px;">APPLY SELECTIONS</button>
    </div>
</nav>
...
function render() {
    const filtered = DATA.filter(p => {
        ...
        return matchSearch && matchDiv && matchStat && matchCat;
    });
    ...
    document.getElementById('mainGrid').innerHTML = filtered.length > 0 ? filtered.map(p => {
        ...
        return `
            <div class="project-card">
                <div class="inst-badge"><i class="fa-solid fa-anchor"></i> MSS</div>
                ...
                <div class="hidden-matrix">
                    ...
                    <div class="matrix-item matrix-dark">
                        <span class="label-text" style="color:#99f6e4"><i class="fa-solid fa-trophy"></i> Achievement Score</span>
        ...
        `;
    }).join('') : '<div style="grid-column:1/-1; text-align:center; padding:50px; opacity:0.5;">No projects found for MSS.</div>';
}
