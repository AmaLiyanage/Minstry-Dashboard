...
$allData = $data ?? [];
$aviationData = $allData['sectors']['aviation'] ?? [];
$projects = [];

// Extract Aviation projects from the multi-sector data structure
foreach ($aviationData as $instCode => $block) {
...
$categories = array_values(array_keys($categories)); sort($categories);
...
<style>
    ...
    .dashboard-header {
        background: linear-gradient(135deg, var(--primary-dark) 0%, #112235 100%);
    ...
    .floating-filter-btn {
        ...
        box-shadow: 0 10px 25px rgba(30, 58, 95, 0.4); display: flex; align-items: center; gap: 10px;
        transition: 0.2s;
    }
    ...
    .field-group label { font-size: 11px; font-weight: 900; color: var(--primary-dark); text-transform: uppercase; display: flex; align-items: center; gap: 8px; }
    ...
    .project-card:hover { transform: translateY(-5px); border-color: var(--primary-dark); box-shadow: 0 15px 30px rgba(0,0,0,0.1); }

    .inst-badge { position: absolute; top: 18px; right: 18px; padding: 5px 12px; border-radius: 8px; font-size: 10px; font-weight: 900; color: white; display: flex; align-items: center; gap: 5px; }
    .bg-AASL { background: #4f46e5; } 
    .bg-CAASL { background: #db2777; }
    ...
    .card-title { font-size: 17px; font-weight: 900; color: var(--primary-dark); margin: 5px 0; line-height: 1.3; padding-right: 75px; min-height: 44px; }
    
    .card-meta-row { display: flex; flex-wrap: wrap; gap: 6px; margin-bottom: 5px; }
    .meta-tag { background: #eff6ff; padding: 4px 10px; border-radius: 6px; font-size: 9px; font-weight: 800; color: var(--primary-dark); border: 1px solid #bfdbfe; display: flex; align-items: center; gap: 5px; }
    ...
    .matrix-dark { background: var(--primary-dark) !important; color: white !important; }
    .matrix-dark .sub-label { color: #93c5fd !important; border-top-color: rgba(255,255,255,0.1) !important;}

    .bar-group { width: 100%; }
    .bar-label { display: flex; justify-content: space-between; font-size: 10px; font-weight: 900; margin-bottom: 4px; align-items: center; color: var(--primary-dark); }
...
<div class="dashboard-header">
    <h1><i class="fa-solid fa-plane-up"></i> AVIATION PROJECTS</h1>
    <div style="font-size: 11px; font-weight: 900; color: #fbbf24;">
        <i class="fa-solid fa-calendar-check"></i> Q1 2026 | <?= strtoupper($selectedOrg) ?> <?= ($selectedDiv != 'all') ? " : ".urldecode($selectedDiv) : "" ?>
    </div>
</div>
...
<nav class="filter-drawer" id="drawer">
    <div class="drawer-content">
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:30px;">
            <h2 style="margin:0; font-size:20px; color: var(--primary-dark);"><i class="fa-solid fa-sliders"></i> Filter Menu</h2>
        ...
        <div class="field-group">
            <label><i class="fa-solid fa-magnifying-glass" style="color:#3b82f6;"></i> Search Project</label>
        ...
        <div class="field-group">
            ...
            <select id="catFilter" class="select-input">
                <option value="all">New & Continuation</option>
        ...
        <div class="field-group">
            <label><i class="fa-solid fa-building-user" style="color:#6366f1;"></i> Institution</label>
            <select id="instFilter" class="select-input">
                <option value="all">Show All</option>
                <option value="AASL" <?= $selectedOrg == 'AASL' ? 'selected' : '' ?>>AASL</option>
                <option value="CAASL" <?= $selectedOrg == 'CAASL' ? 'selected' : '' ?>>CAASL</option>
            </select>
        </div>

        <div class="field-group">
            <label><i class="fa-solid fa-sitemap" style="color:#0ea5e9;"></i> Division</label>
            <select id="divFilter" class="select-input">
                <option value="all">All Divisions</option>
                <?php foreach($divisions as $d): ?>
                    <option value="<?= e($d) ?>" <?= (urldecode($selectedDiv) == $d) ? 'selected' : '' ?>><?= e($d) ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        ...
        <button onclick="toggleDrawer()" style="width:100%; background:var(--primary-dark); color:white; border:none; padding:18px; border-radius:12px; font-weight:900; cursor:pointer; margin-top:10px; font-size:15px;">APPLY SELECTIONS</button>
    </div>
</nav>
...
function isDivisionMatch(projectDiv, filterValue, org) {
    if (filterValue === 'all' || !filterValue) return true;
    if (!projectDiv) return false;
    const pDiv = projectDiv.toUpperCase();
    const fVal = filterValue.toUpperCase();
    if (org === 'AASL') {
        if (fVal === 'CIVIL') return pDiv.includes('CE');
        if (fVal === 'MECH') return pDiv.includes('ME');
    }
    return pDiv === fVal || pDiv.includes(fVal);
}
...
function render() {
    const filtered = DATA.filter(p => {
        ...
        const matchDiv = isDivisionMatch(p.division, filters.div, p.institution_code);
    ...
    document.getElementById('mainGrid').innerHTML = filtered.length > 0 ? filtered.map(p => {
        ...
        return `
            <div class="project-card">
                <div class="inst-badge bg-${p.institution_code}"><i class="fa-solid fa-building"></i> ${p.institution_code}</div>
                ...
                <div class="hidden-matrix">
                    ...
                    <div class="matrix-item">
                        <span class="label-text"><i class="fa-solid fa-vault"></i> Annual Budget (2026)</span>
                    ...
                    <div class="matrix-item matrix-dark">
                        <span class="label-text" style="color:#93c5fd"><i class="fa-solid fa-trophy"></i> Achievement Score</span>
        ...
        `;
    }).join('') : '<div style="grid-column:1/-1; text-align:center; padding:50px; opacity:0.5;">No projects found for this selection.</div>';
}

document.getElementById('searchBox').oninput = e => { filters.query = e.target.value; render(); };
document.getElementById('instFilter').onchange = e => { filters.inst = e.target.value; render(); };
document.getElementById('divFilter').onchange = e => { filters.div = e.target.value; render(); };
