...
WHERE UPPER(TRIM(i.code)) IN ('AASL', 'CAASL') OR UPPER(TRIM(i.institution_name)) LIKE '%AASL%' OR UPPER(TRIM(i.institution_name)) LIKE '%CAASL%'";
...
function render() {
    const filtered = DATA.filter(p => {
        ...
        const matchDiv = isDivisionMatch(pDiv, filters.div, pInst);
    ...
    document.getElementById('mainGrid').innerHTML = filtered.length > 0 ? filtered.map(p => {
        ...
        return `
            <div class="project-card">
                <div class="inst-badge bg-${pInst}"><i class="fa-solid fa-building"></i> ${pInst}</div>
